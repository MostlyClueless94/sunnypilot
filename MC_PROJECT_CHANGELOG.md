...
This file tracks the project-specific history for the BluePilot-based SubiPilot development branch.
It is intentionally separate from the upstream-style `CHANGELOG.md`.
...
- Active repo and install surface: `MostlyClueless94/sunnypilot`
- Active development branch: `MostlyClueless`
- Stock comma installer target: `installer.comma.ai/MostlyClueless94/MostlyClueless`
- Base: `BluePilotDev/bluepilot@bp-6.0`
- Release flow: `MostlyClueless -> subi-staging -> subi-1.0`
- Subaru donor scope: parity with the current `subi-1.0` Subaru support set
- BluePilot remains the credited source for the current Ford baseline and much of the new UI stack.

## 2026-03-27

### `subipilot refresh` BluePilot-based MostlyClueless reset

What changed:
- Rebased `MostlyClueless` development work onto the current `mc-dev` BluePilot snapshot.
- Kept the BluePilot repo shape intact for the first migration wave, including vendored `opendbc_repo`.
- Reintroduced the missing `SUBARU_CROSSTREK_2025` support needed to match the current `subi-1.0` Subaru support baseline.
- Expanded the manual selector exposure for the newer supported Subaru angle-LKAS platforms.
- Started the visible BluePilot-to-SubiPilot rebrand across the development branch while preserving direct BluePilot credit in public-facing docs and release notes.

Why:
- The BluePilot-based branch controlled the Subaru better than the previous SunnyPilot-based SubiPilot development line.
- The cleanest path forward is to make BluePilot the new `MostlyClueless` base, then continue Subaru-specific work from there without disturbing the current public `subi-1.0` line.

Branch/install implications:
- `MostlyClueless` is now the BluePilot-based SubiPilot development branch.
- `subi-staging` and `subi-1.0` stay frozen on the previous public Subaru-first line until the new base validates.
