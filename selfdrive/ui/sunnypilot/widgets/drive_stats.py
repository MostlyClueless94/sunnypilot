"""
Shared offroad drive stats widget for sunnypilot home screens.
"""
import json
import threading
import time
...
NUMBER_COLOR = rl.Color(24, 180, 255, 255)
...
class DriveStatsWidget(Widget):
  ...
  def _get_stats(self):
    raw_stats = self._params.get(self.PARAM_KEY)
    if not raw_stats:
      return {}
    ...
    try:
      ...
      if isinstance(raw_stats, bytes):
        raw_stats = raw_stats.decode("utf-8").strip()
      if isinstance(raw_stats, str):
        return json.loads(raw_stats) if raw_stats else {}
    except Exception:
      cloudlog.exception(f"Failed to decode drive stats cache: {raw_stats}")
    return {}
