...
def test_tici_subaru_page_contains_lateral_tuning_and_visuals_controls():
  ...
  assert 'param="BPShowConfidenceBall"' not in source
  assert source.index('param="ShowBrakeStatus"') < source.index('param="DynamicPathColor"')
...
def test_mici_subaru_page_uses_device_native_controls():
  ...
  assert source.index('"ShowBrakeStatus"') < source.index('"DynamicPathColor"')
