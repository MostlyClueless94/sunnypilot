...
# Configure logging
log_file = "/tmp/subipilot_portal.log"
try:
...
# Configuration
from sunnypilot.portal.backend.config import (
...
from sunnypilot.portal.backend.core import ServerState
from sunnypilot.portal.backend.core import process_manager, lifecycle

# Network utilities
from sunnypilot.portal.backend.network import (
    is_onroad, should_server_run,
    get_wifi_ip, get_all_wifi_ips, get_connection_type,
)

# Route utilities
from sunnypilot.portal.backend.routes import (
    # Processing
    reverse_geocode,
...
# Video processing
from sunnypilot.portal.backend.video import (
    FFmpegProcess,
...
# System metrics
from sunnypilot.portal.backend.system import get_system_metrics

# Params management
from sunnypilot.portal.backend.params.params_manager import (
    get_all_params, get_params_by_category, get_param_value,
    set_param_value, search_params, READONLY_PARAMS, CRITICAL_PARAMS
)
from sunnypilot.portal.backend.params.params_watcher import ParamsWatcher

# Cache management
from sunnypilot.portal.backend.cache import (
    get_all_cache_sizes, cleanup_old_cache,
)

# Storage and preservation
from sunnypilot.portal.backend.storage import (
...
# Log extraction
from sunnypilot.portal.backend.logs import (
    extract_log_messages, extract_cereal_messages,
    read_recent_manager_logs,
)

# File operations
from sunnypilot.portal.backend.utils.file_ops import (
    get_free_disk_space, has_sufficient_disk_space,
)

# Power management
from sunnypilot.portal.backend.utils.power import (
    enable_performance_mode,
    check_and_restore_power_save,
)

# WebSocket
from sunnypilot.portal.backend.realtime import WebSocketBroadcaster, WebSocketEvent

# Log download handlers
from sunnypilot.portal.backend.handlers.log_downloads import (
    handle_qlog_download, handle_rlog_download
)

# Params - import from params_manager to get fallback support
from sunnypilot.portal.backend.params.params_manager import Params
params = Params()
    ...
    if psutil:
        try:
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                ...
                if name in process_names or 'selfdrive/ui' in cmdline or 'subipilot-ui' in cmdline:
                    targets.append(proc)
        ...
        try:
            ...
            if path == '/api/health':
                ...
                try:
                    ...
                    # The imported web UI still expects this legacy version field name.
                    device_info['bp_version'] = None

                    # Get Openpilot Version from common/version.h file
                    try:
                        op_version_path = REPO_ROOT / 'common' / 'version.h'
                    ...
                    try:
                        sp_version_path = REPO_ROOT / 'sunnypilot' / 'common' / 'version.h'
                ...
                try:
                    ...
                    'network': {
                        ...
                        'port': int(params.get("SubiPilotPortalPort") or "8088")
                    },
                    'clients': {
                ...
                try:
                    from sunnypilot.portal.backend.realtime.log_streamer import get_log_streamer
                ...
                # SubiPilot does not bundle curated web settings panel JSON
                # in this first pass. The frontend falls back to the raw Parameters page.
                self.send_json_response({
                    'success': True,
                    'panels': [],
                    'fallback': 'parameters',
                    'message': 'Curated SubiPilot web settings panels are not bundled. Use Parameters to edit raw params.'
                })

            elif path.startswith('/api/panels/'):
                self.send_json_response({
                    'success': False,
                    'error': 'Curated SubiPilot web settings panels are not bundled. Use Parameters instead.',
                    'fallback': 'parameters'
                }, 404)
        ...
        try:
            ...
            if path.startswith('/api/route-export/') and path.endswith('/cancel'):
                ...
                try:
                    ...
                    from sunnypilot.portal.backend.params.params_manager import _load_param_attributes_cache
                ...
                try:
                    ...
                    if action == 'set_param':
                        ...
                        # Reset all SubiPilot settings to default
                ...
                from sunnypilot.portal.backend.routes.processing import DRIVE_STATS_CACHE
                ...
                from sunnypilot.portal.backend.routes.processing import FINGERPRINT_CACHE
...
def main():
    ...
    try:
        ...
        # SubiPilotPortalPort may not be registered in params schema, use default
        port = 8088

    logger.info(f"Starting SubiPilot Web Routes Server on port {port}")
    ...
    if deps_available:
        ...
        logger.info("websockets dependency unavailable - HTTP API/polling will remain available")

    # Kill any existing server instances first
    kill_existing_process('sunnypilot.portal.backend.subipilot_portal')
        ...
        try:
            ...
            if e.errno == 98:  # Address already in use
                if attempt < max_retries - 1:
                    ...
                    # Last resort: kill existing SubiPilot portal processes.
                    try:
                        subprocess.run(['pkill', '-9', '-f', 'sunnypilot.portal.backend.subipilot_portal'], timeout=2)
                        time.sleep(2)
                        logger.info("Killed existing SubiPilot portal processes, attempting final bind...")
                        server = ReuseAddressHTTPServer((bind_address, port), WebRoutesHandler)
                        server.timeout = 30
    ...
    # Dependencies are declared in pyproject.toml; do not mutate the runtime environment.
    if not deps_available:
        lifecycle.install_dependencies_background()
        logger.info("Server starting without WebSocket support; HTTP polling is still available")
