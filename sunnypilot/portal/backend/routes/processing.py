#!/usr/bin/env python3
"""
SubiPilot Route Processing Module

Shared module for route processing logic used by both:
1. The SubiPilot portal process - On-demand processing when routes are accessed
...
METRICS_CACHE = "/data/subipilot/portal/routes/metrics_cache" if os.path.exists("/data") else os.path.expanduser("~/comma_data/subipilot/portal/routes/metrics_cache")
THUMBNAIL_CACHE = "/data/subipilot/portal/routes/thumbs_cache" if os.path.exists("/data") else os.path.expanduser("~/comma_data/subipilot/portal/routes/thumbs_cache")
FINGERPRINT_CACHE = "/data/subipilot/portal/routes/fingerprint_cache" if os.path.exists("/data") else os.path.expanduser("~/comma_data/subipilot/portal/routes/fingerprint_cache")
DRIVE_STATS_CACHE = "/data/subipilot/portal/routes/drive_stats_cache" if os.path.exists("/data") else os.path.expanduser("~/comma_data/subipilot/portal/routes/drive_stats_cache")
...
def reverse_geocode(lat, lon):
    ...
    try:
        ...
        headers = {'User-Agent': 'SubiPilot/1.0 (openpilot route viewer)'}
...
def kill_existing_process(script_name):
    ...
    Args:
        script_name: Name of the Python script or module to match.
    """
    try:
        import signal
