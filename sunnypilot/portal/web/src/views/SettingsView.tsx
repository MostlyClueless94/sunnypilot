...
export function SettingsView({ deviceStatus: _deviceStatus }: SettingsViewProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        link.href = url
        link.download = `subipilot-backup-${new Date().toISOString().split('T')[0]}.json`
  ...
  if (loading && panels.length === 0) {
    return (
      <>
        <Header deviceStatus={_deviceStatus} subtitle="Configure SubiPilot settings and behavior" />
  ...
  if (!loading && panels.length === 0) {
    return (
      <>
        ...
        <div className="settings-view settings-view-centered">
          <div className="settings-error-card">
            <h2>Use Parameters for This Build</h2>
            <p>
              Curated SubiPilot web settings panels are not bundled in this first portal pass.
              Raw parameter review and editing is available from the Parameters page.
            </p>
            <Button variant="primary" onClick={() => navigate('/parameters')}>
              Open Parameters
            </Button>
          </div>
        </div>
  ...
  const headerDescription =
    selectedPanelId === 'favorites'
      ...
      : selectedPanel?.menuDescription || activeMetadata?.description || 'Configure SubiPilot behavior offroad'
